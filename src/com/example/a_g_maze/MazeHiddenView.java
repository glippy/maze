...
public class MazeHiddenView extends View{
	...
	private char[][] lab = new char[m1.WIDTH][m1.HEIGHT];
	
	
	public MazeHiddenView(Context context) {
		super(context);
	}
		...
		if(event.getAction() == MotionEvent.ACTION_DOWN) {
			...
			if (x < W_MAX && y < W_MAX && x > W_MIN && y > W_MIN) {
				if (xCheck < -W || xCheck > W || yCheck < -W || yCheck > W) {
					...
					lab[xWall-2][yWall-2] = m1.checkWay(yWall-2, xWall-2);
					
					if (lab[xWall-2][yWall-2] == '3') {
